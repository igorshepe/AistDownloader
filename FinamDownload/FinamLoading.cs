       ...
       // static readonly Main Main = new Main(new[] { "_" });
        ...
        public static string DownloadData(SecurityInfo security, Main.FileSecurity filesSecurities, Main.SettingsMain settingsData, bool mergeFile)
        {
            ...
            else // Докачка существующих файлов
            {
                ...
                address = $"{Urldownload}{securitySelect.Code}.{"txt"}?d=d&market={securitySelect.MarketId}&em={securitySelect.Id}&p={settings.Period + 1}&df={securityFile.Dat.Day}&mf={securityFile.Dat.Month - 1}&yf={securityFile.Dat.Year}&dt={settings.DateTo.Day}&mt={settings.DateTo.Month - 1}&yt={settings.DateTo.Year}&f={(object) securitySelect.Code}&e=.{"txt"}&datf={5}&cn={(object) securitySelect.Code}&dtf=1&tmf=1&MSOR={settings.TimeCandle}&sep={settings.SplitChar}&sep2=1&at={Convert.ToInt32(settings.FileheaderRow)}";
                Log.Debug("Скачиваю " + address);
